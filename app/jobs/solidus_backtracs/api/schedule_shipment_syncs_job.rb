...
module SolidusBacktracs
  module Api
    class ScheduleShipmentSyncsJob < ApplicationJob
      ...
      def perform
        shipments = SolidusBacktracs::Shipment::PendingApiSyncQuery.apply(::Spree::Shipment.joins(inventory_units: [:variant]).where("spree_variants.sku" => ["SUVIE201R", "SUVIE201RB"]).where.not(shipstation_order_id: nil).distinct)
